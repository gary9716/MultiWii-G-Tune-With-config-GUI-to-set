...
Button buttonIMPORT, buttonSAVE, buttonREAD, buttonRESET, buttonWRITE, buttonCALIBRATE_ACC, buttonCALIBRATE_MAG, buttonSTART, buttonSTOP, buttonSETTING, 
       ...
       btMagDecl,btMWiiHome,btDownloads, buttonTurn_GTUNE_ON_OR_OFF;
...
"Select motor(s) and control throttle.").setPosition(xServ-200, yServ+10).moveTo("Motors");
  ...
  buttonTurn_GTUNE_ON_OR_OFF = controlP5.addButton("TURN_GTUNE_ON_OR_OFF", 1, xGraph+430,yGraph-25, 180, 16).setColorBackground(red_);
...
boolean toggleRead = false,toggleReset = false,toggleCalibAcc = false,toggleCalibMag = false,toggleWrite = false,
        toggleRXbind = false,toggleSetSetting = false,toggleVbat=true,toggleMotor=false,motorcheck=true,toggleGTUNEFlag=false;
...
/*****************************************************************/
    ...
    if(toggleGTUNEFlag) {
      toggleGTUNEFlag = false;
      sendRequestMSP(requestMSP(MSP_TURN_G_TUNE_ON_OR_OFF));
    }
...
public void TURN_GTUNE_ON_OR_OFF() {toggleGTUNEFlag=true;}
...
void InitSerial(float portValue) {
  if (portValue < commListMax) {
    ...
    buttonTurn_GTUNE_ON_OR_OFF.setColorBackground(green_);
    graphEnabled = true;
    g_serial.buffer(256);
    btnQConnect.hide();
  } else {
